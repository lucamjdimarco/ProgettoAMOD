...
assignment_constr = {}
for j in J:
    assignment_constr[j] = model.addConstr(gp.quicksum(x[i, j] for i in range(1, num_J)) == 1, "assignment[%s]" % j)

assignment2_constr = {}
for i in J:
    assignment2_constr[i] = model.addConstr(gp.quicksum(x[i, j] for j in range(1, num_J)) <= 1, "assignment2[%s]" % i)

# ---

assignment3_constr = {}
for j in J:
    assignment_constr[i] = model.addConstr(gp.quicksum(x[i, j] for i in J if j != i) == 1, "assignment[%s]" % i)

assignment4_constr = {}
for i in J:
    assignment_constr[i] = model.addConstr(gp.quicksum(x[i, j] for j in J if j != i) <= 1, "assignment[%s]" % i)

for j in J:
    assignment_excl_constr = model.addConstr(gp.quicksum(x[i, j] for i in J if i != j) == 1, "assignment_excl[%s]" % j)

    assignment_self_excl_constr = model.addConstr(x[j, j] == 0, "assignment_self_excl[%s]" % j)

for j in J:
    assignment_max_excl_constr = model.addConstr(gp.quicksum(x[i, j] for i in J if i != j) <= 1, "assignment_max_excl[%s]" % j)

for j in J:
    assignment_maxJ_excl_constr = model.addConstr(gp.quicksum(x[i, j] for i in J if i != j) <= num_J, "assignment_maxJ_excl[%s]" % j)
