    ...
    for i in J:
        for j in J:
            if(i != j):
                ...
                constraint5_constr[k, i, j] = model.addConstr(C[k, j] >= max(C[k,i], C[k-1, j]) + p[j, k] + bigM * (1 - x[i, j]), "constraint5[%s,%s,%s]" % (k, i, j))
...
for k in M:
    for i in J:
        ...
        for j in J:
                if(i != j):
                    constraint2_constr[k, i, j] = model.addConstr(C[k, i] >= max(C[k,j], C[k-1, i]) + p[i, k] + C[k, j] + bigM * x[i, j], "constraint2[%s,%s,%s]" % (k, i, j))
