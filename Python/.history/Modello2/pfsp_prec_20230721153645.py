...
for i in J:
    for j in J:
        if i < j:
            ...
            model.addConstr(C[1, j] >= p[j, 1] + C[1, i] - M * (1 - x[i, j]), "constraint2[%s,%s,%s]" % (1, i, j))
            model.addConstr(C[1, i] >= p[i, 1] + C[1, j] - M * x[i, j], "constraint3[%s,%s,%s]" % (1, i, j))
            model.addConstr(s1 == C[1, j] - p[j, 1] - C[1, i])
            model.addConstr(s2 == C[1, i] - p[i, 1] - C[1, j])
            model.addConstr(s3 == s1 - M * (1 - x[i, j]))
            model.addConstr(s4 == s2 + M * x[i, j])
