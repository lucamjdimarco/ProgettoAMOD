        ...
        for j in J:
            if i != j:
                ...
                model.addConstr(C[k, j] >= C[k, i] + p[j, k] + bigM * (1 - x[i, j]) + bigM * (1 - y),
                                name="constraint7_aux1[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, j] >= C[k-1, j] + p[j, k] + bigM * (1 - x[i, j]) + bigM * y,
                                name="constraint7_aux2[%s,%s,%s]" % (k, i, j))
                model.addConstr(y <= x[i, j])  # Vincolo di complementarità
        ...
        for j in J:
            if i != j:
                ...
                model.addConstr(C[k, i] >= C[k, j] + p[i, k] + bigM * x[i, j] + bigM * (1 - z),
                                name="constraint8_aux1[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, i] >= C[k-1, i] + p[i, k] + bigM * x[i, j] + bigM * z,
                                name="constraint8_aux2[%s,%s,%s]" % (k, i, j))
                model.addConstr(z <= x[i, j])  # Vincolo di complementarità
