...
for i in J:
    for j in J:
        if i < j:
            model.addConstr(C[1, j] >= p[i, 1] + C[1, i] - bigM * x[i, j], "constraint2[%s,%s,%s]" % (1, i, j))
            model.addConstr(C[1, i] >= p[j, 1] + C[1, j] - bigM * (1-x[i, j]), "constraint3[%s,%s,%s]" % (1, i, j))
    ...
    for i in J:
        for j in J:
            if i < j:
                ...
                model.addConstr(C[k, j] >= p[i, k] + y[k, i, j] - bigM * x[i, j], "constraint2[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, i] >= p[i, k] + c[k, i, j] - bigM * (1-x[i, j]), "constraint3[%s,%s,%s]" % (k, i, j))
