...
for i in J:
    for j in J:
        if i < j:
            s1 = model.addVar(vtype=GRB.CONTINUOUS, name="s1[%s,%s]" % (i, j))
            s2 = model.addVar(vtype=GRB.CONTINUOUS, name="s2[%s,%s]" % (i, j))
            s3 = model.addVar(vtype=GRB.CONTINUOUS, name="s3[%s,%s]" % (i, j))
            s4 = model.addVar(vtype=GRB.CONTINUOUS, name="s4[%s,%s]" % (i, j))
            ...
            model.addConstr(s1 == C[1, j] - p[j, 1] - C[1, i])
            model.addConstr(s2 == C[1, i] - p[i, 1] - C[1, j])
            model.addConstr(s3 == s1 - bigM * (1 - x[i, j]))
            model.addConstr(s4 == s2 + bigM * x[i, j])
            model.addConstr(s3 >= 0)
            model.addConstr(s4 >= 0)
y = {}
c = {}
    ...
    for i in J:
        for j in J:
            if i < j:
                ...
                s1 = model.addVar(vtype=GRB.CONTINUOUS, name="s1[%s,%s,%s]" % (k, i, j))
                s2 = model.addVar(vtype=GRB.CONTINUOUS, name="s2[%s,%s,%s]" % (k, i, j))

                model.addConstr(C[k, j] >= p[j, k] + y[k, i, j] - bigM * (1 - x[i, j]))
                model.addConstr(C[k, i] >= p[i, k] + c[k, i, j] - bigM * x[i, j])

                model.addConstr(s1 == C[k, j] - p[j, k] - y[k, i, j])
                model.addConstr(s2 == C[k, i] - p[i, k] - c[k, i, j])
