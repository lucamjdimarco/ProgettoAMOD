...
for i in J:
    for j in J:
        if(i != j):
            constraint8_constr[1, i, j] = model.addConstr(C[1, j] >= p[j, 1] + C[1, i] + bigM * (1 - x[i, j]), "constraint8[%s,%s,%s]" % (1, i, j))
...
# ---
# assignment_self_excl_constr = {}
# for j in J:
#     assignment_self_excl_constr[j] = model.addConstr(x[j, j] == 0, "assignment_self_excl[%s]" % j)
...
for k in range(2, num_M):
    for i in J:
        ...
        for j in J:
            if(i != j):
                if(gp.greater(C[k, j], C[k-1, i])):
                    constraint2_constr[k, i, j] = model.addConstr(C[k, i] >= C[k,j] + p[i, k] + C[k, j] + bigM * x[i, j], "constraint2[%s,%s,%s]" % (k, i, j))
                else:
                    constraint2_constr[k, i, j] = model.addConstr(C[k, i] >= C[k-1, j] + p[i, k] + C[k, j] + bigM * x[i, j], "constraint2[%s,%s,%s]" % (k, i, j))
...
for i in J:
    ...
    for j in J:
            if(i != j):
                constraint9_constr[1, i, j] = model.addConstr(C[1, i] >= p[i, 1] + C[1, j] + bigM * x[i, j], "constraint2[%s,%s,%s]" % (1, i, j))
