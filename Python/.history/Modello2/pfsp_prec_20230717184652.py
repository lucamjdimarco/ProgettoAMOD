    ...
    for i in J:
        for j in J:
            if i != j:
                ...
                model.addConstr(C[k, i] >= C[k, j] + p[i, k] + bigM * (1 - x[i, j]) + bigM * (1-z), name="constraint8_aux1[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, i] >= C[k-1, i] + p[i, k] + C[k, j] + bigM * x[i, j] + bigM * z, name="constraint8_aux2[%s,%s,%s]" % (k, i, j))
