    ...
    for i in J:
        for j in J:
            if i != j:
                ...
                model.addConstr(C[k, j] >= C[k, i] + p[j, k] + 1e6 * (1 - x[i, j]) + 1e6 * (1 - y))
                model.addConstr(C[k, j] >= C[k - 1, j] + p[j, k] + 1e6 * (1 - x[i, j]) + 1e6 * y)
                constraint7_constr[k, i, j] = y
    ...
    for i in J:
        for j in J:
            if i != j:
                ...
                model.addConstr(C[k, i] >= C[k, j] + p[i, k] + 1e6 * x[i, j] + 1e6 * (1 - z))
                model.addConstr(C[k, i] >= C[k - 1, i] + p[i, k] + 1e6 * x[i, j] + 1e6 * z)
                constraint8_constr[k, i, j] = z
