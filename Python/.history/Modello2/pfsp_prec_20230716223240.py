...
for k in M:
    for i in J:
        for j in J:
            if(i != j):
                constraint5_constr[k, i, j] = model.addConstr(C[k, j] >= p[j, k] + C[k, i] + bigM * (1 - x[i, j]), "constraint1[%s,%s,%s]" % (k, i, j))
...
for m in M:
    for i in J:
        constraint7_constr[m, i] = model.addConstr(C[m, i] >= 0, "constraint6[%s,%s]" % (m, i))
