...
for i in J:
    for j in J:
        if(i != j):
            constraint2_constr[1, i, j] = model.addConstr(C[1, j] >= p[i, 1] + C[1, i] + (bigM * (1 - x[i, j])), "constraint2[%s,%s,%s]" % (1, i, j))
...
for i in J:
    ...
    for j in J:
        if(i != j):
            constraint3_constr[1, i, j] = model.addConstr(C[1, i] >= p[j, 1] + C[1, j] + (bigM * x[i, j]), "constraint3[%s,%s,%s]" % (1, i, j))
    ...
    for i in J:
        for j in J:
            if i != j:
                ...
                model.addConstr(C[k, i] >= C[k, j] + p[i, k] + bigM * x[i, j] + bigM * (1-z), name="constraint8_aux1[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, i] >= C[k-1, i] + p[i, k] + bigM * x[i, j] + bigM * z, name="constraint8_aux2[%s,%s,%s]" % (k, i, j))
