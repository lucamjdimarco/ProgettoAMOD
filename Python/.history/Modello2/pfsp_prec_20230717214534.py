...
for k in range(2, num_M):
    for i in J:
        for j in J:
            if i != j:
                y = model.addVar(lb=0, ub=1, vtype=GRB.CONTINUOUS, name="y[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, j] >= C[k, i] + p[j, k] + bigM * (1 - x[i, j]) + bigM * (1 - y))
                model.addConstr(C[k, j] >= C[k - 1, j] + p[j, k] + bigM * (1 - x[i, j]) + bigM * y)
                model.addConstr(y <= x[i, j])  # Nuovo vincolo di complementarità
                constraint7_constr[k, i, j] = y  # Aggiungi la variabile y ai vincoli
...
for k in range(2, num_M):
    for i in J:
        for j in J:
            if i != j:
                z = model.addVar(lb=0, ub=1, vtype=GRB.CONTINUOUS, name="z[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, i] >= C[k, j] + p[i, k] + bigM * x[i, j] + bigM * (1 - z))
                model.addConstr(C[k, i] >= C[k - 1, i] + p[i, k] + bigM * x[i, j] + bigM * z)
                model.addConstr(z <= x[i, j])  # Nuovo vincolo di complementarità
                constraint8_constr[k, i, j] = z  # Aggiungi la variabile z ai vincoli
