...
#                     constraint7_constr[k, i, j] = model.addConstr(C[k, j] >= C[k,i] + p[j, k] + bigM * (1 - x[i, j]), "constraint7[%s,%s,%s]" % (k, i, j))
#                 else: 
#                     constraint7_constr[k, i, j] = model.addConstr(C[k, j] >= C[k-1, j] + p[j, k] + bigM * (1 - x[i, j]), "constraint7[%s,%s,%s]" % (k, i, j))
    ...
    for i in J:
        for j in J:
            if i != j:
                ...
                model.addConstr(C[k, i] >= C[k-1, j] + 1 - bigM * (1 - y), name="constraint7_aux1[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k-1, j] >= C[k, i] + 1 - bigM * y, name="constraint7_aux2[%s,%s,%s]" % (k, i, j))
...
#                     constraint8_constr[k, i, j] = model.addConstr(C[k, i] >= C[k,j] + p[i, k] + bigM * x[i, j], "constraint8[%s,%s,%s]" % (k, i, j))
#                 else:
#                     constraint8_constr[k, i, j] = model.addConstr(C[k, i] >= C[k-1, i] + p[i, k] + C[k, j] + bigM * x[i, j], "constraint8[%s,%s,%s]" % (k, i, j))
constraint8_constr = {}
for k in range(2, num_M):
    for i in J:
        for j in J:
            if i != j:
                y = model.addVar(vtype=GRB.BINARY, name="y[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, i] >= C[k, j] + p[i, k] + bigM * x[i, j] - bigM * (1 - y), name="constraint8_aux1[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, j] >= C[k-1, i] + p[j, k] + bigM * (1 - x[i, j]) - bigM * y, name="constraint8_aux2[%s,%s,%s]" % (k, i, j))
                constraint8_constr[k, i, j] = model.addConstr(y == 1, name="constraint8[%s,%s,%s]" % (k, i, j))
