...
for k in range(2, num_M):
    for i in J:
        for j in J:
            if i != j:
                y = model.addVar(vtype=GRB.BINARY, name="y[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, i] >= C[k, j] + p[i, k] + bigM * x[i, j] - bigM * y, name="constraint8_aux1[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, i] >= C[k-1, i] + p[i, k] + C[k, j] + bigM * x[i, j] - bigM * (1 - y), name="constraint8_aux2[%s,%s,%s]" % (k, i, j))
