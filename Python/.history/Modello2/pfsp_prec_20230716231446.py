...
for j in J:
    ...
    # oppure
    assignment_constr[j] = model.addConstr(gp.quicksum(x[i, j] for i in range(1, num_J) if(i != j)) <= num_J, "assignment[%s]" % j)

assignment2_constr = {}
for i in J:
    # assignment2_constr[i] = model.addConstr(gp.quicksum(x[i, j] for j in range(1, num_J) if(i != j)) <= 1, "assignment2[%s]" % i)
    # oppure
    assignment2_constr[i] = model.addConstr(gp.quicksum(x[i, j] for j in range(1, num_J) if(i != j)) <= num_J, "assignment2[%s]" % i)
