...
for k in range(2, num_M+1):
    for i in J:
        for j in J:
            if(i < j):
                model.addGenConstrMax(y, [C[k-1, j], C[k,i]], name="max_constraint[%s,%s]" % (i,j))
                model.addGenConstrMax(c, [C[k-1, i], C[k,j]], name="max_constraint2[%s,%s]" % (i,j))

for k in M:
    for i in J:
        for j in J:
            if(i < j):
                model.addConstr((x[i,j] == 1) >> (C[k, j] >= p[j, k] + y), "constraint2[%s,%s,%s]" % (1, i, j))
                model.addConstr((x[i,j] == 0) >> (C[k, i] >= p[i, k] + c), "constraint3[%s,%s,%s]" % (1, i, j))
