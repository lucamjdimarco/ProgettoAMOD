    ...
    for i in J:
        for j in J:
            if i < j:
                ...
                y = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name="y[%s,%s,%s]" % (k, i, j))
                c = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name="c[%s,%s,%s]" % (k, i, j))

                # Vincoli per y e c usando addConstr
                model.addGenConstrMax(y, [C[k - 1, j], C[k, i]], name="max_constraint[%s,%s,%s]" % (k, i, j))
                model.addGenConstrMax(c, [C[k - 1, i], C[k, j]], name="max_constraint2[%s,%s,%s]" % (k, i, j))

                # Vincoli condizionali usando y e c
                model.addConstr((x[i, j] == 1) >> (C[k, j] >= p[j, k] + y), "constraint2[%s,%s,%s]" % (k, i, j))
                model.addConstr((x[i, j] == 0) >> (C[k, i] >= p[i, k] + c), "constraint3[%s,%s,%s]" % (k, i, j))
