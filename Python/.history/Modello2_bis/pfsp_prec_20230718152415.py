    ...
    for i in J:
        for j in J:
            if i < j:
                ...
                model.addConstr(C[k, j] >= C[k, i] + p[j, k] + bigM * (1 - x[i, j]) + bigM * (1-y), name="constraint8_aux1[%s,%s,%s]" % (k, i, j))
                model.addConstr(C[k, j] >= C[k-1, j] + p[j, k] + bigM * (1 - x[i, j]) + bigM * y, name="constraint8_aux2[%s,%s,%s]" % (k, i, j))
