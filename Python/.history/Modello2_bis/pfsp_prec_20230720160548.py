...
for k in M:
    for i in J:
        for j in J:
            if(i < j):
                if x[i,j] == 1:
                    constraint2_constr[k, i, j] = model.addConstr(C[k, j] >= p[j, k] + C[k, i], "constraint2[%s,%s,%s]" % (k, i, j))
                elif x[i,j] == 0:
                    constraint2_constr[k, i, j] = model.addConstr(C[k, i] >= p[i, k] + C[k, j] + (bigM * x[i, j]), "constraint2[%s,%s,%s]" % (1, i, j))
# constraint2_constr = {}
# for k in M:
#     for i in J:
#         for j in J:
