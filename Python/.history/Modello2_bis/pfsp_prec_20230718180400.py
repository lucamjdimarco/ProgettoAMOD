...
#                 model.addConstr(C[k, j] >= Y + p[j, k] + bigM * (1 - x[i, j]) + bigM * (1-y), name="constraint7_aux1[%s,%s,%s]" % (k, i, j))
...
for k in M:
    for i in J:
        for j in J:
            if(i < j):
                constraint2_constr[k, i, j] = model.addConstr(C[k, j] >= p[j, k] + C[1, i] + (bigM * (1 - x[i, j])), "constraint2[%s,%s,%s]" % (1, i, j))
