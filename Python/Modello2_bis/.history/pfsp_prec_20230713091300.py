...
p = {(1, 1): 5, (2, 1): 6, (3, 1): 4, (1, 2): 8, (2, 2): 4, (3, 2): 3, (1, 3): 3, (2, 3): 1, (3, 3): 3, (1, 4): 7, (2, 4): 4, (3, 4): 8}
s = {1: 3, 2: 2, 3: 4}
pi = {1: 1, 2: 2, 3: 3, 4: 4}
...
for k in M:
    for i in J:
        for j in range(i + 1, num_J + 1):
            constraint1_constr[k, i, j] = model.addConstr(C[k, j] >= p[k, j] + C[k, i] - bigM * (1 - x[i, j]), "constraint1[%s,%s,%s]" % (k, i, j))
...
constraint5_constr = {}
for j in J:
    constraint5_constr[j] = model.addConstr(C[1, j] >= p[1, j], "constraint5[%s]" % j)
